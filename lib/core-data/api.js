...
export function getSmartList(prefix, listName, query, size) {
  const mapItems = items => items.map(remoteItem => localItemFromRemote(remoteItem, [{ text: 'value' }]));
  let uri = `${prefix}/rdc/list/${listName}/query/${query}${size ? `?size=${size}` : ''}`;

  assertUri(uri);

  return send(addGetHeader(uri)).then(expectJSONResult).then(res => mapItems(res.items));
}
