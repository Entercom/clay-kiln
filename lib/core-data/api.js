...
import { localItemFromRemote } from '../utils/lists';
...
export function getSmartList(prefix, listName) {
  let uri = `${prefix}/rdc/list/${listName}`;

  assertUri(uri);

  // Get the first page, and if necessary, prepare requests for remaining pages.
  const pageSize = 1;
  let items = [];
  const mapItems = items => items.map(remoteItem => localItemFromRemote(remoteItem, [{text: 'value'}]));
  const getPage = page => {
    console.log(`getPage(${page})`);
    return send(addGetHeader(`${uri}?page=${page}&size=${pageSize}`)).then(expectJSONResult)
      .then(r => {
        console.log(`getPage(${page}) result`, r);
        return r;
      });
  };
  return getPage(1).then(res => {
    items = mapItems(res.items);
    console.log(`mapItems -> `, items);
    const pages = Math.ceil(res.totalEntries / pageSize);
    ...
    for (let page = 2; page <= pages; page++) {
      promises.push(getPage(page).then(r => {
        items = [...items, ...mapItems(r.items)];
      }));
    }
...
export function addSmartListItem(prefix, listName, item) {
  let url = `${prefix}/rdc/list/${listName}/add`;
...
export function removeSmartListItem(prefix, listName, item) {
  let url = `${prefix}/rdc/list/${listName}/reduce`;
