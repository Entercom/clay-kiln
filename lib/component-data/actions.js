...
import {
  UPDATE_COMPONENT, REVERT_COMPONENT, REMOVE_COMPONENT, CURRENTLY_SAVING, OPEN_ADD_COMPONENT, CLOSE_ADD_COMPONENT, UPDATE_SCHEMA_PROP, CURRENTLY_RESTORING, COMPONENT_ADDED
} from './mutationTypes';
import { getData, getSchema } from '../core-data/components';
import {
  refProp, refAttr, layoutAttr, editAttr, componentListProp, componentProp, getComponentName
...
import {
  getComponentEl, getParentComponent, getPrevComponent, isComponentInPage, isComponentInHead
} from '../utils/component-elements';
...
function logSaveError(uri, e, data, eventID, snapshot, store) { // eslint-disable-line
  log.error(`Error saving component (${getComponentName(uri)}): ${e.message} : ${e.response}`, { action: 'saveComponent', data });
  store.dispatch('finishProgress', 'error');
  store.dispatch('showSnackbar', {
    message: `Error saving ${label(getComponentName(uri))}`,
    action: 'Retry',
    onActionClick: () => store.dispatch('saveComponent', {
...
/**
 * trigers the onComponentSaveError kiln plugin if it exists
 *
 * @param {string} type - either 'client' or 'server'
 * @param {object} data - data intended to help with debugging the error
 */
function onComponentSaveErrorPlugin(type, data) {
  if (!window.kiln.onComponentSaveError) {
    return;
  }

  try {
    window.kiln.onComponentSaveError(type, data);
  } catch (err) {
    log.error(
      'Error triggering onComponentSaveError plugin',
      {
        data,
        error: err.stack,
        type
 ...
 * @return {Promise}
 */
function revertReject({
  uri, data, snapshot, paths, store
}) {
 ...
 * @return{promise}
 */
export function updateSchemaProp(store, {
  schemaName, inputName, prop, value
}) {
  return store.commit(UPDATE_SCHEMA_PROP, {
    schemaName, inputName, prop, value
  });
}
  ...
  return modelSave(uri, data)
    ...
    .then((savedData) => {
      ...
      addToQueue(save, [uri, savedData, false], 'save') // add hooks=false to prevent models from re-running on server
        ...
        .then(res => {
          if (_.get(window.kiln, 'locals.causeServerComponentSaveError') === 'true') {
            return Promise.reject(
              new Error("causeServerPageSaveError set to 'true'")
            );
          }
        ...
        .catch((e) => {
          ...
          return revertReject({
            uri, data: oldData, snapshot, paths, store
          });
        });
    ...
    .then((renderableData) => {
      ...
      return store.dispatch('render', {
        uri, data: renderableData, snapshot, paths
      });
    })
    .then(res => {
      if (_.get(window.kiln, 'locals.causeClientComponentSaveError') === 'true') {
        return Promise.reject(
          new Error("causeClientPageSaveError set to 'true'")
        );
      }
 ...
 * @return {Promise}
 */
export function saveComponent(store, {
  uri, data, eventID, snapshot, prevData, forceSave = false
}) {
 ...
 * @returns {Promise}
 */
function addComponentsToComponentList(store, data, {
  currentURI, parentURI, path, replace, components
}) {
 ...
 * @returns {Promise}
 */
function addComponentsToPageArea(store, {
  currentURI, path, replace, components
}) {
  ...
  return create(components)
    .then((newComponents) => {
      return store.dispatch('addCreatedComponentsToPageArea', {
        newComponents, store, currentURI, path, replace, index, data
      });
    });
}
 ...
 * @returns {Promise}
 */
export function addCreatedComponentsToPageArea(store, {
  newComponents, currentURI, path, replace, index, data, forceRender
}) {
 ...
 */
export function addComponents(store, {
  currentURI, parentURI, path, replace, components
}) {
  if (replace !== true) {
 ...
 * @returns {Promise}
 */
export function openAddComponent(store, {
  currentURI, parentURI, path, pos
}) {
