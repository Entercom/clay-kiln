...
const MODEL_SAVE_TIMEOUT = 16000, // timeout for model.js pre-save hook
  MODEL_RENDER_TIMEOUT = 4000; // timeout for model.js pre-render hook. should be smaller than pre-save
...
export function save(uri, data) {
  ...
  if (_.isObject(model) && _.isFunction(model.save)) {
    return timeout(attempt(() => model.save(uri, _.cloneDeep(data), getLocals())), MODEL_SAVE_TIMEOUT);
  } else {
...
export function render(uri, data) {
  ...
  if (_.isObject(model) && _.isFunction(model.render)) {
    return timeout(attempt(() => model.render(uri, _.cloneDeep(data), getLocals())), MODEL_RENDER_TIMEOUT);
  } else {
    return Promise.resolve(data);
  }
}
